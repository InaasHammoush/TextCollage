...
//   SOME OTHER OPTIONS THAT CAN BE APPLIED TO TEXT ITEMS:
	...
	public JMenuBar getMenuBar() {
		if (menuBar == null) {
			...
			JMenuItem randomCollageItem = new JMenuItem("Create a random collage...");
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			File ImageInfo = fileChooser.getOutputFile(this, "Select Image Info File Name", "imageInfo.txt");
			try {
				PrintWriter infoOut = new PrintWriter(ImageInfo);
				infoOut.println(canvas.getBackground().getRed());
				infoOut.println(canvas.getBackground().getGreen());
				infoOut.println(canvas.getBackground().getBlue());
				for (DrawTextItem item : theString) {
					infoOut.println(item.getString());
					infoOut.println(item.getX());
					infoOut.println(item.getY());
					Font f = item.getFont();
					infoOut.println(f.getName());
					infoOut.println(f.getStyle());
					infoOut.println(f.getSize());
					infoOut.println(item.getMagnification());
					infoOut.println(item.getBorder());
					infoOut.println(item.getRotationAngle());
					infoOut.println(item.getTextTransparency());
					Color c = item.getBackground();
					infoOut.println(c.getRed());
					infoOut.println(c.getGreen());
					infoOut.println(c.getBlue());
					infoOut.println(item.getBackgroundTransparency());
					Color tc = item.getTextColor();
					infoOut.println(tc.getRed());
					infoOut.println(tc.getGreen());
					infoOut.println(tc.getBlue());
				}
			...
			File infoIn = fileChooser.getInputFile();
			try {
				Scanner infoInReader = new Scanner(infoIn);
				Color bgc = new Color(Integer.parseInt(infoInReader.nextLine()),
						Integer.parseInt(infoInReader.nextLine()),
						Integer.parseInt(infoInReader.nextLine()));
				canvas.setBackground(bgc);
				while (infoInReader.hasNext()) {
					DrawTextItem item = new DrawTextItem(infoInReader.nextLine(),
							Integer.parseInt(infoInReader.nextLine()),
							Integer.parseInt(infoInReader.nextLine()));
					Font f = new Font(infoInReader.nextLine(), Integer.parseInt(infoInReader.nextLine()),
							Integer.parseInt(infoInReader.nextLine()));
					item.setFont(f);
					item.setMagnification(Double.parseDouble(infoInReader.nextLine()));
					item.setBorder(Boolean.parseBoolean(infoInReader.nextLine()));
					item.setRotationAngle(Double.parseDouble(infoInReader.nextLine()));
					item.setTextTransparency(Double.parseDouble(infoInReader.nextLine()));
					Color c = new Color(Integer.parseInt(infoInReader.nextLine()),
							Integer.parseInt(infoInReader.nextLine()),
							Integer.parseInt(infoInReader.nextLine()));
					item.setBackground(c);
					item.setBackgroundTransparency(Double.parseDouble(infoInReader.nextLine()));
					Color tc = new Color(Integer.parseInt(infoInReader.nextLine()),
							Integer.parseInt(infoInReader.nextLine()),
							Integer.parseInt(infoInReader.nextLine()));
		...
		// this is the extra option I added to create a random collage.
		else if (command.equals("Create a random collage...")) {
			File randomCollageFile = fileChooser.getInputFile(this, "Select a text file to create a random "
					+ "collage of its components.");
			try {
				Scanner reader = new Scanner(randomCollageFile);
				ArrayList<String> randomStrings = new ArrayList<>();
				while (reader.hasNext()) {
					randomStrings.add(reader.nextLine());
				}
				Collections.shuffle(randomStrings);
				for (String string : randomStrings) {
					DrawTextItem item = new DrawTextItem(string, (int)(Math.random()*750),
							(int)(Math.random()*550)); //I used 50 pixels less than the width and height
					                                   //to avoid having strings too close to the border
					theString.add(item);
				}
				canvas.repaint();
				reader.close();
